...
export default function Waves() {
  ...
  return (
    <div className="h-full min-h-0 overflow-hidden sm:py-3 lg:px-2">
      <div className="relative h-full min-h-0 overflow-hidden rounded-none border border-cyan-100/70 bg-white/90 shadow-[0_28px_70px_-34px_rgba(8,145,178,0.35)] sm:rounded-[28px] dark:border-slate-700/70 dark:bg-slate-900/85">
          ...
          <section className={`absolute inset-0 z-10 flex min-h-0 flex-col border-r border-cyan-100/80 bg-white/80 transition-transform duration-300 ease-out dark:border-slate-700/80 dark:bg-slate-900/60 md:relative md:inset-auto md:z-auto md:w-[340px] md:translate-x-0 xl:w-[360px] ${mobileView === 'thread' ? '-translate-x-full' : 'translate-x-0'}`}>
            <div className="shrink-0 border-b border-cyan-100/80 bg-white/90 px-5 py-5 backdrop-blur dark:border-slate-700/80 dark:bg-slate-900/90">
              ...
              <div className="mt-4 flex items-center gap-3 rounded-2xl border border-cyan-100 bg-slate-50/80 px-4 py-3 dark:border-slate-700 dark:bg-slate-950/50">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search messages" className="w-full bg-transparent text-sm text-slate-700 outline-none placeholder:text-slate-400 dark:text-slate-100" />
              </div>
            </div>
                ...
                <div className="min-h-0 flex-1 overflow-hidden">
                  <div className="flex h-full min-h-0 overflow-hidden">
                          ...
                          {activeMessages.map((message) => {
                            ...
                            return (
                              <div key={message.id} className={`flex items-end gap-3 ${outgoing ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className={`max-w-[82%] rounded-[26px] px-4 py-3 shadow-sm sm:max-w-[70%] ${outgoing ? 'bg-gradient-to-r from-surf-primary to-cyan-500 text-white' : 'border border-cyan-100/80 bg-white text-slate-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100'}`}>
                    ...
                    {showInfo && activeConversation.peer && (
                      <aside className="hidden w-[340px] shrink-0 border-l border-slate-800 bg-slate-950 text-slate-100 xl:flex xl:flex-col">
                        <div className="border-b border-slate-800 px-6 py-5"><h3 className="text-xl font-semibold">Thông tin hội thoại</h3></div>
                        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-6">
                          <WaveAvatar src={activeConversation.peer.avatarUrl} name={activeConversation.peer.name} className="mx-auto h-24 w-24 rounded-full border border-slate-700 object-cover" fallbackClassName="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-surf-primary to-cyan-500 text-2xl font-semibold text-white" />
                          <h4 className="mt-4 text-center text-3xl font-semibold text-white">{activeConversation.peer.name}</h4>
                          <p className="mt-2 text-center text-sm text-slate-400">Direct message · Surf Waves</p>
                          <div className="mt-6 grid grid-cols-3 gap-3">
                            <button type="button" onClick={() => setMuteConversation((current) => !current)} className="rounded-2xl bg-slate-900 px-3 py-3 text-center hover:bg-slate-800"><div className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-slate-800 text-cyan-300"><svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor"><path d="M12 3a9 9 0 0 0-9 9h2a7 7 0 1 1 7 7v2a9 9 0 0 0 0-18Zm1 5h-2v5.41l3.3 3.3 1.4-1.42-2.7-2.7Z" /></svg></div><p className="mt-2 text-xs font-medium text-slate-200">{muteConversation ? 'Bật thông báo' : 'Tắt thông báo'}</p></button>
                            <button type="button" className="rounded-2xl bg-slate-900 px-3 py-3 text-center hover:bg-slate-800"><div className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-slate-800 text-cyan-300"><svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor"><path d="m16 3 5 5-9 9H7v-5Zm-1.4 2.8L9 11.4V15h3.6l5.6-5.6Z" /></svg></div><p className="mt-2 text-xs font-medium text-slate-200">Ghim hội thoại</p></button>
                            <button type="button" className="rounded-2xl bg-slate-900 px-3 py-3 text-center hover:bg-slate-800"><div className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-slate-800 text-cyan-300"><svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor"><path d="M15 12a4 4 0 1 0-4-4 4 4 0 0 0 4 4Zm-8 0a3 3 0 1 0-3-3 3 3 0 0 0 3 3Zm8 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4Zm-8 0c-.34 0-.72.02-1.12.06C4.01 14.28 1 15.44 1 18v2h4v-2c0-1.18.56-2.18 1.54-3A8.72 8.72 0 0 1 7 14Z" /></svg></div><p className="mt-2 text-xs font-medium text-slate-200">Tạo nhóm trò chuyện</p></button>
                          </div>
                          <div className="mt-6 space-y-4">
                            <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-5">
                              <div className="flex items-center justify-between text-sm text-slate-300"><span>Danh sách nhắc hẹn</span><span className="rounded-full bg-slate-800 px-2 py-1 text-[11px] text-slate-400">0</span></div>
                              <div className="mt-4 flex items-center justify-between text-sm text-slate-300"><span>Nhóm chung</span><span className="rounded-full bg-slate-800 px-2 py-1 text-[11px] text-slate-400">0</span></div>
                              <div className="mt-4 rounded-2xl bg-slate-950 px-4 py-3 text-sm text-slate-300">
                                <p className="text-xs uppercase tracking-[0.18em] text-cyan-300">Last signal</p>
                                <p className="mt-2 font-medium text-white">{formatFullTime(activeConversation.lastMessageAt)}</p>
                                <p className="mt-2 text-slate-400">{activeConversation.lastMessagePreview ?? 'Chưa có tín hiệu mới trong thread này.'}</p>
                              </div>
                            </div>
                              ...
                              {sharedMedia.length > 0 ? (
                                <div className="grid grid-cols-3 gap-3">
                                  {sharedMedia.slice(0, 6).map((item) => (
                                    <a key={item.url} href={item.url} target="_blank" rel="noreferrer" className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950"><img src={item.url} alt={item.label} className="h-24 w-full object-cover" /></a>
                                  ))}
                                </div>
                              ) : <p className="text-sm text-slate-400">Chưa có ảnh hoặc video nào được chia sẻ.</p>}
                            </InfoSection>
                              ...
                              {sharedFiles.length > 0 ? (
                                <div className="space-y-3">
                                  {sharedFiles.slice(0, 4).map((item) => (
                                    <a key={item.url} href={item.url} target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-2xl bg-slate-950 px-4 py-3 hover:bg-slate-800">
                                      <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-cyan-500/15 text-cyan-300"><svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor"><path d="M14 2H6a2 2 0 0 0-2 2v16l4-2 4 2 4-2 4 2V8Zm0 1.5L19.5 9H14Z" /></svg></div>
                                      <div className="min-w-0"><p className="truncate text-sm font-medium text-white">{item.label}</p><p className="mt-1 text-xs text-slate-400">{item.hostname}</p></div>
                                    </a>
                                  ))}
                                </div>
                              ) : <p className="text-sm text-slate-400">Chưa có tệp nào được chia sẻ.</p>}
                            </InfoSection>
                              ...
                              {sharedPages.length > 0 ? (
                                <div className="space-y-3">
                                  {sharedPages.slice(0, 5).map((item) => (
                                    <a key={item.url} href={item.url} target="_blank" rel="noreferrer" className="flex items-start gap-3 rounded-2xl bg-slate-950 px-4 py-3 hover:bg-slate-800">
                                      <div className="mt-0.5 flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-800 text-cyan-300"><svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor"><path d="M3.9 12a5 5 0 0 1 1.47-3.54l2.12-2.12a5 5 0 0 1 7.07 7.07l-1.06 1.06-1.41-1.41 1.06-1.06a3 3 0 1 0-4.24-4.24L6.79 9.88a3 3 0 1 0 4.24 4.24l.53-.53 1.41 1.41-.53.53A5 5 0 0 1 3.9 12Zm6.54.71 3.15-3.15 1.41 1.41-3.15 3.15Z" /></svg></div>
                                      <div className="min-w-0"><p className="truncate text-sm font-medium text-white">{item.label}</p><p className="mt-1 truncate text-xs text-cyan-300">{item.hostname}</p><p className="mt-1 truncate text-xs text-slate-500">{item.url}</p></div>
                                    </a>
                                  ))}
                                </div>
                              ) : <p className="text-sm text-slate-400">Chưa có liên kết nào trong cuộc trò chuyện này.</p>}
                            </InfoSection>

                            <InfoSection title="Thiết lập bảo mật" open={infoSections.security} onToggle={() => toggleSection('security')}>
                              <div className="space-y-4 text-sm text-slate-200">
                                <div className="flex items-center justify-between rounded-2xl bg-slate-950 px-4 py-3"><div><p className="font-medium">Tin nhắn tự xóa</p><p className="mt-1 text-xs text-slate-400">Hiện đang để Không bao giờ</p></div><span className="rounded-full bg-slate-800 px-2 py-1 text-[11px] text-slate-400">Off</span></div>
                                <button type="button" onClick={() => setHideConversation((current) => !current)} className="flex w-full items-center justify-between rounded-2xl bg-slate-950 px-4 py-3 text-left"><div><p className="font-medium">Ẩn trò chuyện</p><p className="mt-1 text-xs text-slate-400">Che thread này khỏi danh sách nhanh</p></div><span className={`relative h-6 w-11 rounded-full transition ${hideConversation ? 'bg-cyan-500' : 'bg-slate-700'}`}><span className={`absolute top-0.5 h-5 w-5 rounded-full bg-white transition ${hideConversation ? 'left-[22px]' : 'left-0.5'}`} /></span></button>
