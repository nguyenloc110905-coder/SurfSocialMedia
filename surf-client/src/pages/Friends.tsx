...
function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={`bg-white dark:bg-gray-900/70 border border-gray-200/60 dark:border-gray-700/50 rounded-3xl p-4 flex items-center gap-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 ${className}`}
    >
      {children}
    </div>
  );
...
function GradBtn({
  ...
  const base =
    'inline-flex items-center gap-1.5 px-4 h-9 rounded-2xl text-sm font-semibold transition-all disabled:opacity-50';
  const styles = {
    primary:
      'bg-gradient-to-r from-surf-primary to-surf-secondary text-white shadow-sm hover:shadow-surf-primary/30 hover:scale-[1.02] active:scale-[0.98]',
    ghost:
...
export default function Friends() {
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off('friendRequestReceived', onReq);
    };
  }, [user]);
  ...
  const handleAccept = async (requestId: string) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Lỗi chấp nhận.');
    } finally {
      setActioningId(null);
    }
  };
  ...
  const handleAddFriend = async (toUid: string, name: string, avatarUrl?: string) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Lỗi gửi lời mời.');
    } finally {
      setActioningId(null);
    }
  };
  ...
  const handleCancelSent = async (requestId: string, toUid: string) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Lỗi hủy lời mời.');
    } finally {
      setActioningId(null);
    }
  };
  ...
  function AddFriendBtn({
    ...
    if (hasRequest(uid)) {
      ...
      return (
        <GradBtn
          ...
          onClick={() => req && handleAccept(req.id)}
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
            <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" />
          </svg>
    ...
    if (hasSent(uid)) {
      ...
      return (
        <GradBtn
          variant="ghost"
          disabled={actioningId === reqId}
          onClick={() => handleCancelSent(reqId, uid)}
        >
          {actioningId === reqId ? (
            <Spinner />
          ) : (
    ...
    return (
      <GradBtn
        variant="primary"
        disabled={actioningId === uid}
        onClick={() => handleAddFriend(uid, name, avatarUrl)}
      >
        {actioningId === uid ? (
          <Spinner />
        ) : (
        ...
        <div>
          ...
          {section === 'requests' && (
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {requests.length > 0
                ? `${requests.length} lời mời nhận được`
        ...
        <>
                        ...
                        <li key={s.id}>
                          <Card>
                            ...
                            <div className="flex-1 min-w-0">
                              <Link
                                to={`/feed/profile/${s.id}`}
                                className="font-semibold text-gray-900 dark:text-gray-100 hover:text-surf-primary transition-colors truncate block"
                              >
                                {s.name}
                              </Link>
                              {isFriend(s.id) && (
                                <p className="text-xs text-emerald-500">Bạn bè của bạn</p>
                              )}
                              {hasSent(s.id) && !isFriend(s.id) && (
                                <p className="text-xs text-surf-primary dark:text-surf-secondary">
                                  Đang chờ xác nhận
                                </p>
                              )}
                            </div>
                      ...
                      <li key={s.id}>
                        <Card>
                          ...
                          <GradBtn
                            ...
                            onClick={() => handleCancelSent(s.id, s.toUid)}
                          >
          ...
          {section === 'suggestions' &&
            (suggestions.length === 0 ? (
              ...
              (() => {
                const firstNonMutualIndex = suggestions.findIndex((s) => (s.mutualCount ?? 0) <= 0);
                ...
                (() => {
                  ...
                  return filtered.length === 0 ? (
                    <p className="text-sm text-gray-500 text-center py-8">
                      Không tìm thấy &ldquo;{friendsSearchQ}&rdquo;
                    </p>
                  ) : (
                    <ul className="space-y-2">
                      {filtered.map((f) => (
                          ...
                          <Card>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {f.mutualCount != null && f.mutualCount > 0 && (
                                <p className="text-xs text-gray-400 dark:text-gray-500">
                                  {f.mutualCount} bạn chung
                                </p>
                              )}
                            </div>
                            <Link
                              to="/feed/waves"
                              className="inline-flex items-center gap-1.5 px-4 h-9 rounded-2xl text-sm font-semibold bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
                            >
                              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
          ...
          {section === 'history' && (
            <EmptyState
              icon={
                <svg className="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M13 3c-4.97 0-9 4.03-9 9H1l3.89 3.89.07.14L9 12V6c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.93 0-3.68-.79-4.94-2.06l-1.42 1.42C8.27 19.99 10.51 21 13 21c4.97 0 9-4.03 9-9s-4.03-9-9-9zm-1 5v5l4.28 2.54.72-1.21-3.5-2.08V8H12z" />
                </svg>
              }
