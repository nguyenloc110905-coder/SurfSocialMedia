...
export default function CustomSettingsModal({ onDone, onClose }: CustomSettingsModalProps) {
  const [customSettingsValues, setCustomSettingsValues] = useState<
    Record<string, CustomSettingValue>
  >(() =>
    Object.fromEntries(
      CUSTOM_SETTINGS_ITEMS.map(({ key }) => [key, key === 'follow_list' ? 'only_me' : 'public'])
    )
  );
    ...
    <>
      ...
      >
        <div
          ...
          aria-hidden
          onClick={() => !editingSettingKey && onClose()}
        />
        <div className="relative flex flex-col w-full max-w-lg rounded-2xl bg-white dark:bg-slate-800 border-2 border-surf-primary/30 dark:border-surf-secondary/30 shadow-2xl">
          <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-600 rounded-t-2xl bg-gradient-to-r from-surf-primary/10 to-transparent dark:from-surf-secondary/15">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-2 rounded-lg text-slate-500 hover:text-surf-primary dark:hover:text-surf-secondary transition-colors"
              aria-label="Đóng"
            >
          ...
          <div className="flex-shrink-0 px-4 py-4 space-y-1">
            {CUSTOM_SETTINGS_ITEMS.map(({ key, title }) => (
              ...
              >
                ...
                <span className="text-sm text-slate-500 dark:text-slate-400 flex-shrink-0">
                  {CUSTOM_SETTING_LABELS[customSettingsValues[key] ?? 'public']}
                </span>
                <svg
            ...
            <div className="pt-3 mt-3 border-t border-slate-200 dark:border-slate-600">
              <p className="text-sm text-slate-700 dark:text-slate-200 mb-2">
                Cho phép công cụ tìm kiếm liên kết đến trang cá nhân Surf?
              </p>
              <button
                type="button"
                role="switch"
              ...
              >
                <span
                  className={`pointer-events-none inline-block h-6 w-6 transform rounded-full bg-white shadow ring-0 transition ${searchEngineLink ? 'translate-x-5' : 'translate-x-1'}`}
                />
              </button>
              <span className="ml-2 text-sm text-slate-500 dark:text-slate-400">
                {searchEngineLink ? 'Bật' : 'Tắt'}
              </span>
            </div>
          </div>
