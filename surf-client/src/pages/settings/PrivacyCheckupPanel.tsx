    ...
    <div className="max-w-3xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        {PRIVACY_CHECKUP_TOPICS.map((topic) => (
          ...
          >
            <span
              className={`w-12 h-12 rounded-xl bg-gradient-to-br ${topic.accent} flex items-center justify-center flex-shrink-0 shadow-sm group-hover:scale-105 transition-transform [&_svg]:!text-white [&_svg]:w-6 [&_svg]:h-6 dark:opacity-95`}
            >
            ...
            <svg
              className="w-5 h-5 text-slate-300 dark:text-slate-500 group-hover:text-surf-primary dark:group-hover:text-surf-secondary flex-shrink-0 self-center transition-colors"
              viewBox="0 0 24 24"
              fill="currentColor"
            >
      ...
      <p className="text-sm text-slate-500 dark:text-slate-400">
        ...
        <button
          type="button"
          onClick={onBack}
          className="text-surf-primary dark:text-surf-secondary font-medium hover:underline"
        >
          Cài đặt
        </button>
        .
