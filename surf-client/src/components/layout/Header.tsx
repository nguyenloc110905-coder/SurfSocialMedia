...
export default function Header({ hideCenterNav = false }: HeaderProps) {
  ...
  const avatarSeed =
    (storeDisplayName ?? user?.displayName)?.trim() || user?.email?.trim() || 'Người dùng';
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/95 dark:bg-gray-900/95 backdrop-blur border-b border-gray-200 dark:border-gray-700 safe-area-header flex items-center justify-between h-12 sm:h-14 px-3 sm:px-6 lg:px-8 gap-2 sm:gap-4 relative">
      ...
      <div
        className={`flex items-center gap-2 sm:gap-4 flex-1 min-w-0 ${hideCenterNav ? 'max-w-[25%]' : 'max-w-[50%]'}`}
      >
        <Link to="/feed" className="flex-shrink-0">
          <img
            src="/DashboardLogo.png"
            alt="Surf"
            className="h-7 sm:h-9 w-auto object-contain dark:invert dark:brightness-90"
          />
        </Link>
        <SearchBox
          wrapperClassName={`hidden sm:flex flex-1 min-w-[80px] ${hideCenterNav ? 'max-w-[140px] md:max-w-[160px] lg:max-w-[200px]' : 'max-w-[260px] sm:max-w-[300px] lg:max-w-[340px]'}`}
        />
      </div>
      ...
      <div
        className="flex items-center justify-end gap-2 sm:gap-3 lg:gap-5 flex-shrink-0 min-w-0"
        ref={menuRef}
      >
        <button
          type="button"
        ...
        <button
          type="button"
          onClick={() => {
            setMenuPath(open ? null : location.pathname);
            ...
            >
              ...
              <div className="w-80 flex-shrink-0 py-2 max-h-[70vh] overflow-y-auto">
                ...
                <div className="px-3 py-3">
                  ...
                  <button
                    type="button"
                    className="w-full flex items-center gap-3 py-3 text-left hover:bg-gray-100 dark:hover:bg-gray-700 active:bg-gray-100 dark:active:bg-gray-600 transition-colors rounded-lg px-1 mt-2"
                  >
                    <span className="w-9 h-9 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center flex-shrink-0 text-gray-700 dark:text-gray-200">
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M20 5H4c-1.1 0-1.99.9-1.99 2L2 17c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm-9 3h2v2h-2V8zm0 3h2v2h-2v-2zM8 8h2v2H8V8zm0 3h2v2H8v-2zm-1 2H5v-2h2v2zm0-3H5V8h2v2zm9 5H8v-2h8v2zm0-3h-2v-2h2v2zm0-3h-2V8h2v2zm3 3h2v2h-2v-2zm0-3h2V8h-2v2z" />
                      </svg>
                    </span>
                  ...
                  <button
                    type="button"
                    className="w-full flex items-center gap-3 py-3 text-left hover:bg-gray-100 dark:hover:bg-gray-700 active:bg-gray-100 dark:active:bg-gray-600 transition-colors rounded-lg px-1"
                  >
                    <span className="w-9 h-9 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center flex-shrink-0 text-gray-700 dark:text-gray-200">
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M16 4c0-1.11.89-2 2-2s2 .89 2 2-.89 2-2 2-2-.89-2-2zm4 18v-6h2.5L12 4 2.5 22H5v-6h3v6h2v-6h2v6h2v-6h4z" />
                      </svg>
                    </span>
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100">
                      Cài đặt trợ năng
